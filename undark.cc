...
\------------------------------------------------------------------*/
int decode_row(struct globals *g, char *p, char *data_endpoint, struct sql_payload *payload, int mode,
    ...
    if (payload->header_size > g->page_size)
        return 0; // sorry, no can do with the way we're playing this decoding game.
    ...
    while (1) {
        ...
        switch (s) {
            ...
            case 10:
            case 11:
                DEBUG fprintf(stdout, "%s:%d:DEBUG: celltype 10/11 reserved, aborting row.\n", FL);
                s = 0;
                return 0;
                break;
            default:
        ...
        DEBUG {
            fprintf(stdout, "[%d:%d:%d-%d(%d)]", t, payload->cells[t].t, payload->cells[t].s, payload->cells[t].o,
                    plh_ep - p);
        }
    ...
    DEBUG fprintf(stdout, "Offset [%u] + headersize [%lu] = length check [%lu]... \n", offset,
                  (unsigned long int) payload->header_size, (unsigned long int) payload->length);
...
int dump_row(struct globals *g, char *base, char *data_endpoint, struct sql_payload *payload, int mode) {
    ...
    if (payload->overflow_pages[0] == 0) {
        ...
        while (payload->overflow_pages[ovpi]) {
            ...
            addr = g->db_origin + ((payload->overflow_pages[ovpi] - 1) * g->page_size) +
                   4; //PLD:20141221-2240 segfault fix
            if ((addr < (void *) g->db_origin) || (addr > (void *) g->db_end)) {
                DEBUG fprintf(stdout,
                              "%s:%d:dump_row:ERROR: page seek request outside of boundaries of file (%p < %p > %p)\n",
        ...
        if (t >= 0) {
            ...
            switch (payload->cells[t].t) {
                case 0:
                    ...
                    break;
                case 1:
                    fprintf(stdout, "x%d", to_signed_byte(*(payload->mapped_data + payload->cells[t].o)));
                    break;
                case 2: {
                    uint16_t n;
                ...
                case 5:
                    fprintf(stdout, "%d", ntohl(*(payload->mapped_data + payload->cells[t].o)));
                    break;
                case 6:
                    fprintf(stdout, "%d", ntohl(*(payload->mapped_data + payload->cells[t].o)));
                    break;
                ...
                case 12:
                    if (g->report_blobs) {
                        if (payload->cells[t].s < g->blob_size_limit) {
                            ...
                            blob_dump((unsigned char *) (payload->mapped_data + payload->cells[t].o),
                                      payload->cells[t].s);
                        } else {
...
char *find_next_row(struct globals *g, char *s, char *end_point, char *global_start, int mode, size_t forced_length) {
    ...
    do {
        ...
        if (row) {
            DEBUG fprintf(stdout, "ROWID: %ld found [+%ld] record size: %d bytes\n", (unsigned long int) sql.rowid,
                          p - global_start, (unsigned int) (sql.length + sql.prefix_length));
            fflush(stdout);
    ...
    while (g->db_cfp < g->db_end) {
        ...
        DEBUG {
            ...
            fprintf(stdout, "%s:%d:Dumping main block in RAW... [ Page No: %lu, Offset: %lu (0x%X),  size : %d ]\n", FL,
                    (long unsigned int) g->page_number, (long unsigned int) (g->db_cpp - g->db_origin),
                    (unsigned int) (g->db_cpp - g->db_origin), g->page_size
            );
        ...
        if (*(g->db_cfp) == 13) {
            ...
            DEBUG fprintf(stdout, "%s:%d:DEBUG: Freeblock offset = %u, size = %u, next block = %u \n", FL,
                          leaf.freeblock_offset, leaf.freeblock_size, leaf.freeblock_next);
            if (leaf.freeblock_size > 0) {
                DEBUG fprintf(stdout, "%s:%d:DEBUG: Freeblock data [ %d bytes total [4 bytes for header] ]\n", FL,
                              leaf.freeblock_size);
                DEBUG hdump((unsigned char *) (g->db_cfp + leaf.freeblock_offset + 4), leaf.freeblock_size - 4,
                            "Actual data in free block");
            }
            fflush(stdout);
            ...
            DEBUG fprintf(stdout,
                          "%s:%d:DEBUG: PAGEHEADER:%d pagebyte: %d, freeblock offset: %d, cell count: %d, first cell offset %d, free bytes %d\n",
                          FL, leaf.page_number, leaf.page_byte, leaf.freeblock_offset, leaf.cellcount, leaf.cell_offset,
                          leaf.freebytes
                );
            ...
            if (g->freelist_space_only) {
                ...
                if ((leaf.freeblock_offset > 0) && (leaf.freeblock_size > 0)) {
                    ...
                    DEBUG hdump((unsigned char *) g->db_cfp - 4, 32,
                                "Scratch pointer at freespace data start (including 4 byte header)");
                    DEBUG fflush(stdout);
                }
            }
